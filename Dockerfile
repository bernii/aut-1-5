...
RUN apt-get update && apt install fonts-dejavu-core rsync git jq moreutils vim procps -y && apt-get clean
...
RUN --mount=type=cache,target=/root/.cache/pip  pip install -U opencv-python-headless fastapi==0.90.1 requests==2.28.2 runpod==0.8.4
...
WORKDIR ${ROOT}
RUN git apply /docker/auto1111.patch
RUN PYTHONPATH=/stable-diffusion-webui python /docker/prefetch.py --ckpt /docker/sd-v1-5-inpainting.ckpt

EXPOSE 7860
ENTRYPOINT ["/docker/entrypoint.sh"]
CMD python -u /docker/reluncher.py
